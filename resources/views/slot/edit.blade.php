...
@section('content')
    ...
    <h2>Update the Slot</h2>
    <form id="slotForm" action="{{route('slot.update',$data->id)}}" method="POST">
        @csrf
        @method('PUT')
        <div class="row g-3">
            <!-- Route Select -->
            <div class="col-md-6">
                ...
                <select class="form-select select2" id="route_id" name="route_id">
                    ...
                    @foreach($route as $r)
                        <option value="{{$r->id}}" {{$data->route_id==$r->id?'selected':''}}>{{$r->start_location}} to {{$r->end_location}}</option>
                    @endforeach
                </select>
                @error('route_id')
            ...
            <div class="col-md-6">
                ...
                <select class="form-select select2" id="bus_id" name="bus_id">
                    ...
                    @foreach($bus as $b)
                        <option value="{{$b->id}}" {{$data->bus_id==$b->id?'selected':''}} >{{$b->bus_name}} - {{$b->bus_code}}</option>
                    @endforeach
                </select>
                @error('bus_id')
                <span class="text-danger">{{$message}}</span>
            ...
            <div class="col-md-6">
                ...
                <select class="form-select select2" id="driver_id" name="driver_id">
                    ...
                    @foreach($driver as $d)
                        <option value="{{$d->id}}" {{$data->driver_id==$d->id?'selected':''}}>{{$d->name}} - {{$d->driver_code}}</option>
                    @endforeach
                </select>
                @error('driver_id')
                <span class="text-danger">{{$message}}</span>
            ...
            <div class="col-md-6">
                ...
                <input type="text"  placeholder="Select date & time" class="form-control" id="schedule" name="schedule" value="{{$data->schedule}}">
                @error('schedule')
                <span class="text-danger">{{$message}}</span>
                @enderror
            </div>
        ...
        <div class="mt-4 d-flex justify-content-center">
            <button type="submit" class="btn btn-primary btn-lg">Update Slot</button>
        </div>
    </form>
